  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col font-sans">
      ...
      <form onSubmit={onSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-7 space-y-6">
            <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
              <h3 className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">Banking</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="text-left text-gray-500 uppercase border-b border-gray-100">
                      <th className="pb-2 font-bold w-1/4">Address banking</th>
                      <th className="pb-2 font-bold w-1/5">Account Name</th>
                      <th className="pb-2 font-bold w-1/4">Account Number</th>
                      <th className="pb-2 font-bold w-1/6">Status</th>
                      <th className="pb-2 font-bold w-1/6">Note</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td className="py-2 pr-1"><input name="bankingAddress" value={form.bankingAddress} onChange={onChange} className={inputClass} /></td>
                      <td className="py-2 pr-1"><input name="bankingAccountName" value={form.bankingAccountName} onChange={onChange} className={inputClass} /></td>
                      <td className="py-2 pr-1"><input name="bankingAccountNumber" value={form.bankingAccountNumber} onChange={onChange} className={inputClass} /></td>
                      <td className="py-2 pr-1"><input name="bankingStatus" value={form.bankingStatus} onChange={onChange} className={inputClass} /></td>
                      <td className="py-2"><input name="bankingNote" value={form.bankingNote} onChange={onChange} className={inputClass} /></td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        {/* ROW 3: Contract - 2 Cột bên trong giống Modal */}
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          <h3 className="text-xs font-bold text-gray-500 mb-4 uppercase tracking-wider">Contract</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
            <div className="space-y-1">
              <FormRow label="Contract Name *" required><input name="contractName" value={form.contractName} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Contract Type *" required><input name="contractType" value={form.contractType} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Salary Basic *" required><input name="salaryBasic" value={form.salaryBasic} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Branch *" required><input name="branch" value={form.branch} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Staff Type *" required><input name="staffType" value={form.staffType} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="End Day"><input type="date" name="endDay" value={form.endDay} onChange={onChange} className={dateClass} /></FormRow>
              <FormRow label="Note"><textarea name="note" value={form.note} onChange={onChange} rows={2} className={inputClass} /></FormRow>
            </div>
            <div className="space-y-1">
              <FormRow label="Contract Number *" required><input name="contractNumber" value={form.contractNumber} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Salary Gross"><input name="salaryGross" value={form.salaryGross} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Salary Capacity *" required><input name="salaryCapacity" value={form.salaryCapacity} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Department *" required><input name="department" value={form.department} onChange={onChange} className={inputClass} /></FormRow>
              <FormRow label="Payment Method *" required><input name="paymentMethod" value={form.paymentMethod} onChange={onChange} className={inputClass} /></FormRow>
            </div>
          </div>
        </div>
