    ...
    <div className="space-y-6">
      ...
      <div className="border border-gray-200 rounded-xl p-6 min-h-[600px] relative flex flex-col">
        ...
        <div className="space-y-4 flex-1">
          {posts.map((post) => (
            <div key={post.id} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
...
function CreateAnnouncementView({ onBack, onSave }: { onBack: () => void, onSave: (post: Post) => void }) {
  ...
  const handleSave = () => {
    ...
    const newPost: Post = {
      ...
      title: title,
      content: content,
      author: "You (Admin)", // Giả định người dùng hiện tại
      time: "Just now",
      likes: 0,
      comments: 0,
      views: 0,
