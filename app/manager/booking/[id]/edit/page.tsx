...
export default function EditBookingPage() {
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-border">
          ...
          <div className="mt-4">
            <Button asChild variant="outline" type="button">
              <Link href="/manager/booked-rooms">Back</Link>
            </Button>
          </div>
        </div>
      </div>
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      router.push("/manager/booked-rooms");
    } catch (err) {
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_460px]">
        <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-border">
          ...
          <div className="mt-2 text-sm text-muted-foreground">
            Capacity: <span className="font-medium text-foreground">{room?.capacity ?? booking?.room?.capacity ?? "?"}</span>
          </div>
