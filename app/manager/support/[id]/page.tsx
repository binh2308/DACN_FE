...
function normalizeProcessesResponse(raw: any): {
  ...
  if (raw?.data && Array.isArray(raw.data.processes)) {
    return {
      ...
      total: Number(
        raw.data.total_activities ?? raw.data.processes.length ?? 0,
      ),
    };
  }
  ...
  if (Array.isArray(raw))
    return { processes: raw as TicketProcessDto[], total: raw.length };
...
export default function TicketDetailPage() {
  ...
  const statusInfo =
    STATUS_CONFIG[normalizedTicketStatus] || STATUS_CONFIG.OPEN;
  const StatusIcon = statusInfo.icon;
  const employeeName =
    formatPersonName(ticket.employee) || "Người dùng ẩn danh";
            ...
            <CardContent className="p-6 md:p-8">
                ...
                <div className="space-y-0 relative before:absolute before:inset-0 before:ml-[1.125rem] md:before:ml-28 before:-translate-x-px before:h-full before:w-0.5 before:bg-border">
                  {processes.map((p, index) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`relative flex items-start gap-4 ${!isLast ? "pb-8" : ""} group`}
                      >
                        {/* Thời gian (Desktop) */}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div
                            className={`p-4 rounded-xl ring-1 ${isSystem ? "bg-background ring-border" : "bg-primary/5 ring-primary/20"}`}
                          >
              ...
              {processesError ? (
                <div className="mt-4 rounded-lg bg-destructive/10 p-3 text-xs text-destructive ring-1 ring-destructive/20">
                  ...
                  {(processesError as any)?.message || "Lỗi không xác định"}
                </div>
              ) : null}
            </CardContent>
          </Card>
              ...
              {normalizedTicketStatus === "OPEN" ? (
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Chuyển trạng thái từ &quot;Chờ xử lý&quot; sang &quot;Đang
                    xử lý&quot;.
                  </div>
                  <Textarea
                    value={approveNote}
                ...
                <div className="mt-3 flex items-center gap-3 text-sm">
                  {ticket.assignee ? (
                    ...
                    <div className="text-muted-foreground italic flex items-center gap-2">
                      <UserCircle2 className="h-4 w-4" /> Chưa có người nhận xử
                      lý
                    </div>
                  )}
                </div>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {employeesError ? (
                    <div className="rounded-lg bg-destructive/10 p-3 text-xs text-destructive ring-1 ring-destructive/20">
                      ...
                      {(employeesError as any)?.message || "Lỗi không xác định"}
                    </div>
                  ) : null}
                </div>
              </div>
